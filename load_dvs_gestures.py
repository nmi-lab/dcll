...
class sequence_generator_sorted(sequence_generator):
    ...
    def __next__(self):
        ...
        if self.shuffle:
            ...
            if self.pointer>self.n_total*self.chunk_size*self.batch_size:
        ...
        if self.shuffle:
            ...
            for k,bk in sequence_size.items():
                if bk>0:
                    ...
                    idx = (self.base.T[:,count:count+bk]+np.arange(0,bk)).T + self.pointer - np.cumsum([0]+self.n)[k]
                    d = self.x[k][idx].reshape(-1,self.chunk_size,input_shape[0])
                    mbx[count:count+bk] = d
                    mbt[count:count+bk] = k
                    count+=bk
        assert count == self.batch_size, "count is {0} should be {1}".format(count, self.batch_size)
...
def create_data(valid = False, chunk_size=CHUNK_SIZE, batch_size=100):
    ...
    gen_train = sequence_generator_sorted(targets_train,data_train,chunk_size = chunk_size, batch_size = batch_size, shuffle = True)

    targets_test = np.load('dataset/test_pol_streaming1000_labels.npy')
    data_test = np.load('dataset/test_pol_streaming1000.npy')
    gen_test = sequence_generator_sorted(targets_test, data_test, chunk_size = chunk_size, shuffle=True, batch_size = batch_size)
