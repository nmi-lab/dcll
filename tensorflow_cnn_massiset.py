...
layers[1], states[1], output1 = DCNNConvLayer(feat_out=Nfeat2,
        ksize=7,
        input_shape=[input_width//2,input_height//2,Nfeat1],
        target_size = Nout,
        layer_input = output0   ,
        batch_size=batch_size,
        tau = 10,
...
layers[2], states[2], output2 = DCNNConvLayer(feat_out=Nfeat2,
        ksize=7,
        input_shape=[input_width//2,input_height//2,Nfeat2],
        target_size = Nout,
        layer_input = output1   ,
        batch_size=batch_size,
        tau = 10,
...
layers[3], states[3], output2 = DCNNConvLayer(feat_out=Nfeat3,
        ksize=7,
        input_shape=[input_width//2,input_height//2,Nfeat2],
        target_size = Nout,
        layer_input = output1   ,
...
if __name__ == '__main__':
    ...
    for i in range(nepochs):
        ...
        accs = [np.mean(p[BURNIN:].cumsum(axis=0).argmax(axis=2)==targets_original[BURNIN:].cumsum(axis=0).argmax(axis=2)) for p in ps]
